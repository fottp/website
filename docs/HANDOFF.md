...
**Theme upkeep (checked 2026-09-19):** the theme (`devcows/hugo-universal-theme`) is in maintenance mode — last commit March 2026 (two commits in the previous 12 months, both from outside contributors), last release Oct 2024, the original maintainer's last commit July 2025, 60+ open issues/PRs with the recent ones unreviewed. The submodule is on its latest commit. Rather than fork it (the underlying template's licence forbids redistributing derivatives, and a fork would still need the attribution), fixes go in the site's own `layouts/`, which override the theme's. Hugo is pinned at 0.166.0 in CI, so deprecated features the theme uses can't break the build until that is bumped. Cleanup done so the build has **no deprecation warnings**: `.Site.LanguageCode` → `.Site.Language.Locale` in our layouts and in local copies of the theme's `404.html` and `page/single.html`; `layouts/index.html` no longer calls the theme's unused config-driven sections (carousel, features, testimonials, see-more, recent posts, clients — they rendered nothing and used the deprecated `.Site.Data`); and `disableKinds = ['taxonomy', 'term']` in `hugo.toml` removes the empty `/categories/` and `/tags/` pages (nothing linked to them) and the sidebar/search widget they used. Check `hugo --logLevel warn` after any Hugo or theme bump.
...
- `layouts/_markup/render-image.html` runs for every Markdown image. A JPEG/PNG **in the page's own folder** becomes a responsive `<img>` via `layouts/partials/responsive-img.html`; anything else (remote address, `/static` path, SVG, GIF) is output as a plain `<img>`, as before.
- `responsive-img.html` makes 480 / 800 / 1200px copies (only ones smaller than the original, never enlarged; quality 82) and offers them plus the original in a `srcset`; the browser picks one using `sizes` and the screen. `loading="lazy"` defers photos below the fold. News posts use a narrower `sizes` (their column is narrower). The news thumbnails and lead image use the same partial (`news-thumb.html`, `news/single.html`); thumbnails set `noOriginal` so a phone never picks the full-size file.
