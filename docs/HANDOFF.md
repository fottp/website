...
**Real photos added 2026-09-15** (gruntfutuk's own, resized to a 1600px max edge and recompressed before committing — originals were 2-8MB camera/phone files): two general park shots on the home page, a real "Out and about" photo on About Us (resolving that TODO), and a new "The Park We're Protecting" section on Our Projects with photos of the Abraham Darby monument, the old chimney, and a fishing lake. Two more photos were held back pending checks with gruntfutuk: one showing runners in Santa hats on a muddy path (unclear if it's an actual FOTTP-organised event or an unrelated public fun run) and a Facebook-sourced snowy photo (`FB_IMG_...` filename suggests it may not be gruntfutuk's own — provenance/rights unconfirmed).
