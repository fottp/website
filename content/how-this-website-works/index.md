...
- **Forms:** [Web3Forms](https://web3forms.com/) — a proprietary, closed-source hosted service for processing form submissions (its client-side helper libraries are open source, but the backend service itself is not), run by a company based in Kerala, India, on server infrastructure in the US. It doesn't keep submissions: each one is forwarded straight on by email, and its own server logs are cleared after about two months. No third-party tracking scripts run on our pages. It's a small, self-contained piece of the site — just two forms, referenced by a couple of configuration keys — so it could be swapped for an alternative without touching anything else
